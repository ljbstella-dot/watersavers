...
async function saveSaving(dateKey, liters) {
  // Firestore에 저장
  await saveSavingOnline(dateKey, liters);
  // 캐시 갱신
  if (savingsCache) savingsCache[dateKey] = liters;
}
...
async function renderPiggyWeekly() {
  ...
  document.getElementById('piggy-weekly-summary').innerHTML = `
    ...
    <div class="piggy-sum-item">
      <div class="piggy-sum-label">참여 일수</div>
      <div class="piggy-sum-val">${weekDates.filter(d => savings[d.key] > 0).length} 일</div>
    </div>
  `;
}
