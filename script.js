...
async function renderPiggyDaily() {
  ...
  if (labelEl) labelEl.textContent = `${year}년 ${monthNames[month]}`;
  ...
  if (summaryEl) {
    summaryEl.innerHTML = monthTotal > 0
      ? `<i class="fa-solid fa-droplet"></i> ${year}년 ${monthNames[month]} 합계 <strong>${monthTotal.toLocaleString()} L</strong>`
      : `<span style="color:var(--text-muted)">${year}년 ${monthNames[month]}에는 기록이 없습니다.</span>`;
  }
}
...
async function openPage(name) {
  ...
  // 체크 패널: 최신 인기도 갱신 + 제출 여부 확인 후 버튼 상태 설정
  if (name === 'check') {
    const btn = document.getElementById('btn-check-confirm');
    btn.disabled = true;
    btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> 확인 중...';
...
// ── 팝업 처리 ─────────────────────────────────────────────
const POPUP_KEY = 'noShowPopup_';

function showPopupIfNeeded(uid) {
  const key = POPUP_KEY + uid;
  if (localStorage.getItem(key) === 'true') return;
  document.getElementById('popup-notice').style.display = '';
}
...
function handleNoShow(chk) {
  const user = getCurrentUser();
  if (!user) return;
  const key = POPUP_KEY + user.uid;
...
window.handleNoShow  = handleNoShow;

// ── 시작 ──────────────────────────────────────────────────
// initApp()은 onAuthChanged 내부에서 로그인 후 호출됩니다.
