...
/* ============================================================
   ...
   NEIS API: ATPT_OFCDC_SC_CODE=B10, SD_SCHUL_CODE=7010902
   ============================================================ */
...
const SCHOOL_CONFIG = {
  ...
  // KEY가 있어도 브라우저 직접 호출 시 CORS 오류 → 프록시 유지
  PROXY: 'https://api.allorigins.win/raw?url=',
  API_BASE: 'https://open.neis.go.kr/hub/mealServiceDietInfo'
};
...
function loadSavings() {
  try { return JSON.parse(localStorage.getItem('waterSavings') || '{}'); }
  catch { return {}; }
}
function saveSaving(dateKey, liters) {
  const data = loadSavings();
  data[dateKey] = (data[dateKey] || 0) + liters;
  localStorage.setItem('waterSavings', JSON.stringify(data));
}
...
async function fetchMeal(dateStr) {
  const url = `${SCHOOL_CONFIG.API_BASE}?KEY=${SCHOOL_CONFIG.KEY}&Type=json&pIndex=1&pSize=20`
    + `&ATPT_OFCDC_SC_CODE=${SCHOOL_CONFIG.ATPT}`
    + `&SD_SCHUL_CODE=${SCHOOL_CONFIG.CODE}`
    + `&MLSV_YMD=${dateStr}`;

  const proxyUrl = SCHOOL_CONFIG.PROXY + encodeURIComponent(url);
...
async function confirmCheck() {
  ...
  if (savedToday > 0) saveSaving(getToday(), savedToday);
  updateRecords();
  updateStatMini();
...
function renderPiggyWeekly() {
  ...
  document.getElementById('piggy-weekly-summary').innerHTML = `
    ...
    <div class="piggy-sum-item">
      ...
      <div class="piggy-sum-val">${getBestWeekly().toLocaleString()} L</div>
    </div>
  `;
}

function renderPiggyDaily() {
  const savings = loadSavings();
...
function switchPiggyTab(tab) {
  ...
  if (tab === 'weekly') renderPiggyWeekly();
  else renderPiggyDaily();
}

// ── 최고 기록 ─────────────────────────────────────────────
function getBestWeekly() {
  const s = loadSavings();
  return getWeekDates().reduce((total, d) => total + (s[d.key] || 0), 0);
}
function updateRecords() {
  const recData = JSON.parse(localStorage.getItem('waterRecords') || '{}');
  const todayVal = loadSavings()[getToday()] || 0;
  const weekTotal = getBestWeekly();
  if (!recData.daily || recData.daily < todayVal) {
    recData.daily = todayVal;
    recData.dailyDate = getToday();
  }
...
async function openPage(name) {
  ...
  if (name === 'piggy') renderPiggyWeekly();
